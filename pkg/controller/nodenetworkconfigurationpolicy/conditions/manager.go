...
func (m *Manager) refreshPolicyConditions() error {
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if failing > 0 {
			message := fmt.Sprintf("%d/%d nodes failed to configure", failing, numberOfNodes)
			setPolicyFailedToConfigure(&m.policy.Status.Conditions, message)
		} else if progressing > 0 {
			message := fmt.Sprintf("%d/%d nodes configuring", progressing, numberOfNodes)
			setPolicyProgressing(&m.policy.Status.Conditions, message)
		} else if numberOfNodes > numberOfEnactments {
			message := fmt.Sprintf("%d/%d nodes not started to configure yet", numberOfNodes-numberOfEnactments, numberOfNodes)
			setPolicyProgressing(&m.policy.Status.Conditions, message)
		} else if available == numberOfNodes {
			message := fmt.Sprintf("All %d nodes successfully configured", numberOfNodes)
			setPolicySuccess(&m.policy.Status.Conditions, message)
		} else {
			message := fmt.Sprintf("None of %d nodes matches the policy", numberOfNodes)
			setPolicyNotMatching(&m.policy.Status.Conditions, message)
		}
